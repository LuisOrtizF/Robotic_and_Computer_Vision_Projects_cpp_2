                   ...
                   PointCloud<PointXYZ> &keypoints_tgt)
{
  ...
  uniform.setRadiusSearch (0.3f);  // 1m
                 ...
                 PointCloud<Normal> &normals_tgt)
{
  ...
  normal_est.setRadiusSearch (0.003f);  // 50cm
              ...
              PointCloud<FPFHSignature33> &fpfhs_tgt)
{
  ...
  fpfh_est.setRadiusSearch (0.008f); // 1m
                          ...
                          Correspondences &remaining_correspondences)
{
  ...
  rej.setMaximumDistance (0.5f);    // 1m
                       ...
                       Eigen::Matrix4f &transform)
{
  ...
  print_info ("\nFound %lu and %lu keypoints for the source and target datasets.\n", keypoints_src->points.size (), keypoints_tgt->points.size ());
  ...
  print_info ("\nEstimated %lu and %lu normals for the source and target datasets.\n", normals_src->points.size (), normals_tgt->points.size ());
  ...
  for (int i = 0; i < good_correspondences->size (); ++i){
    std::cerr << "\nGood Correspondences: "<< good_correspondences->at (i) << std::endl;
  }
  ...
  viscorr.addPointCloud(src, "src_points"); 
  Eigen::Matrix4f t; 
  t<<1,0,0,5, 
     0,1,0,0, 
     0,0,1,0, 
  ...
  pcl::transformPointCloud(*tgt,cloudview,t); 

  viscorr.addPointCloud(cloudview.makeShared(), "tgt_points"); 

  for (size_t i =0; i <good_correspondences->size(); i++) 
  { 
    const pcl::PointXYZ & p_src = keypoints_src->points.at(good_correspondences->at(i).index_query); 
    pcl::PointXYZ & p_tgt = keypoints_tgt->points.at(good_correspondences->at(i).index_match); 
    ...
    viscorr.addLine(p_src, p_tgt, 0, 0, 255, ss.str ());                
  } 
  viscorr.resetCamera (); 
  viscorr.spin (); 
}
...
int main (int argc, char** argv)
{
  ...
  std::cerr << "\nRelative Transform: " << transform << std::endl;
